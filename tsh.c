...
void eval(char *cmdline) 
{
    ...
    int bgOrfg = parseline(cmdline, arguments);
    int background = builtin_cmd(arguments);
    if(background != 0){
        return;
    }
    ...
    if((childpid = fork()) == 0){
        // Each process will have the same pid and pgid.
        int r = setpgid(0,0);
        if(r < 0){
            fprintf(stderr, "Error occured with setpgid(0,0)\n");
        }


        sigprocmask(SIG_SETMASK, &prev_one, NULL); /* Unblock SIGCHLD */
        int error = 0;
        error = execve(arguments[0], &arguments[0], NULL);
        if(error == -1){
            // No command of arguments[0] found
            printf("%s: Command not found\n", arguments[0]);
        }
    }
    else{
        // Add the job here in the parent process
        int state;
        if(bgOrfg){
            state = BG;
        }
        ...
        addjob(jobs, childpid, getpgid(childpid), state, cmdline);
        sigprocmask(SIG_SETMASK, &prev_one, NULL);

        if(bgOrfg){
            printf("[%d] (%d) %s", getjobpid(jobs, childpid) -> jid, (int) childpid, cmdline);
        }
        else{
            waitfg(childpid);
        }
...
void do_bgfg(char **argv) 
{
    if(!strcmp(argv[0],"bg")){
        ...
        if('%' == argv[1][0]){
            ...
            // Check to see if it is a valid jobID
            if(jobid == 0){
                printf("not a real number after %% symbol.\n");
            ...
            printf("[%d] (%d) %s", getjobjid(jobs, jobid) -> jid, (int) getjobjid(jobs, jobid) -> pid, getjobjid(jobs, jobid) -> cmdline);
            getjobpid(jobs, getjobjid(jobs, jobid) -> pid) -> state = BG;
            kill(-1 * (getjobjid(jobs, jobid) -> pid), SIGCONT);
            waitfg(getjobjid(jobs, jobid) -> pid);
        }
        else if(atoi(&argv[1][0]) != 0){
            // Possibly a valid number, futher testing required.
            int jobid = pid2jid ( (pid_t)atoi(&argv[1][0]));
            if(jobid == 0){
        ...
        if('%' == argv[1][0]){
            ...
            getjobpid(jobs, getjobjid(jobs, jobid) -> pid) -> state = FG;
            kill(-1 * (getjobjid(jobs, jobid) -> pid), SIGCONT);
            waitfg(getjobjid(jobs, jobid) -> pid);
        }
        else if(atoi(&argv[1][0]) != 0){
...
void sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &wstatus, WNOHANG|WUNTRACED)) > 0) {
        if (WIFEXITED(wstatus)) {
            ...
            // Sigint
            printf("Job [%d] (%d) terminated by signal %d\n", getjobpid(jobs, pid) -> jid, (int) pid, WTERMSIG(wstatus));
        } else if (WIFSTOPPED(wstatus)) {
            // Sigtstp
            getjobpid(jobs, fgpid(jobs)) -> state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", getjobpid(jobs, pid) -> jid, (int) pid, WSTOPSIG(wstatus));
            continue;
